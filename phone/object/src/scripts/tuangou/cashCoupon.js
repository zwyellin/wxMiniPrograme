$(function(){
	...
	Cash.prototype={
		...
		getCoupon:function(opts){
			...
			var url="merchant/h5callback/findGroupPurchaseCouponInfo?groupPurchaseCouponId="+opts.id+"&longitude="+opts.longitude+"&latitude="+opts.latitude+"";
			var ajaxdata='';
	...
	//	        "loop":true,
			...
			document.title=res.groupPurchaseMerchant.name+"代金券";
			//价格
			var pricestr='';
			pricestr+='<div class="price">';
			pricestr+='<span>￥<b>'+res.price+'</b></span>';
			pricestr+='<span>代￥<i>'+res.originPrice+'</i></span>';
			pricestr+='</div>';
			...
			//商家推荐
			var marketRecomd=marketMsg.merchantRecommend||'';
			if(marketRecomd!=''){
				$('.special_goods').html(marketRecomd);
			}else if(marketRecomd==''){
